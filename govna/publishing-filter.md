...
`W-YEAR` warns a sentence that holds both a first-person word and an exact year. It ignores link addresses and bare URLs, and a year in a neighboring sentence does not count. It also skips table rows, where a title such as "Episode I" is not the word "I".
