...
Use this table as the record of triage verdicts on candidate entries. `publishing-filter.md` `## Triage` owns the rules. One row per verdict the Director confirmed, newest first. The label is a short neutral phrase, never the raw thought. Detail holds the target entry for a `Merge`, the condition for a `Park`, or the reason for a `Drop`. Entry stays empty until the entry ships.
...
| 2026-09-22 | Publish | Flourishing in six directions, pursued as a loop where direction beats arrival | `life/`, `take`, "Flourishing" | `life/flourishing.md` |
